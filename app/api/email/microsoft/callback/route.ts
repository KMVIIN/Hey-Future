...
import { EMAIL_COOKIE, MICROSOFT_SCOPES, sealEmailSession } from "@/lib/email";
...
export async function GET(request: Request) {
  ...
  const expectedState = cookieStore.get("future_email_oauth_state")?.value;
  ...
  if (!clientId || !clientSecret || !process.env.EMAIL_SESSION_SECRET) {
    return NextResponse.redirect(`${origin}/?email=config`);
  }
  ...
  response.cookies.set(EMAIL_COOKIE, sealed, {
    httpOnly: true,
    secure: true,
    sameSite: "lax",
    maxAge: 60 * 60 * 24 * 30,
