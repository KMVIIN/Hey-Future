...
import { EMAIL_COOKIE, GOOGLE_SCOPES, sealEmailSession } from "@/lib/email";
...
export async function GET(request: Request) {
  ...
  const expectedState = store.get("future_google_oauth_state")?.value;
  ...
  response.cookies.set(EMAIL_COOKIE, sealed, { httpOnly: true, secure: true, sameSite: "lax", maxAge: 60 * 60 * 24 * 30, path: "/" });
